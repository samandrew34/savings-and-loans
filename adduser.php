...
$stmt = $conn->prepare("SELECT can_add, hide_add FROM permit WHERE (userid = ? OR email = ?) AND links = 'add_user.php'");
$stmt->bind_param("ss", $userid, $email);
$stmt->execute();
$result = $stmt->get_result();
$permission = $result->fetch_assoc();
$canAdd = $permission['can_add'] ?? 0;
$hideAdd = $permission['hide_add'] ?? ''; // Get the comma-separated list of user IDs

// Check if current user is in the hide_add list
$restrictedUsers = explode(',', $hideAdd); // Convert the string to an array of user IDs
if (in_array($userid, $restrictedUsers)) {
    $canAdd = 0; // Deny permission if user is in the restricted list
}
if ($canAdd == 0) {
    echo "<script>
        setTimeout(function() {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $customerEmail = trim($_POST['email']);  // Added email field
    $customeremail = trim($_POST['customeremail']);//the useremail in users_tbl
    ...
    $insertStmt = $conn->prepare("INSERT INTO customerdata (userid, workid, rank, firstname, surname, othername, maritalStatus, options, gender, email,customeremail,customernumber,connumber,idnumber,fullname,location) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?, ?,?,?,?,?)");
    $insertStmt->bind_param("ssssssssssssssss", $userid, $workid, $rank, $firstname, $surname, $othername, $maritalStatus, $option, $gender, $customerEmail,$customeremail,$customernumber,$connumber,$idnumber,$fullname,$location);
